...
function changeSlide(sign) {
   ...
   const Innerdots = document.querySelectorAll('.Reviews_slider-items_slick-dot');
   for(var numIn = 0; numIn < InnerSlides.length; numIn++){
       InnerSlides[numIn].classList.remove('active'); 
       InnerSlides[numIn].classList.remove('rightAnimSh'); 
       InnerSlides[numIn].classList.remove('leftAnimSh'); 
   }
    for (var i = Innerdots.length - 1; i >= 0; i--) {
        Innerdots[i].classList.remove('activeDot'); // убираем активность всех точек при переключении слайда вн
    }
...
slidesInner[0].classList.add("active");
...
function changeDot(index) {
    ...
    console.log("indexSlideInner = " +indexSlideInner);
